...
def generate_and_post():
    ...
    prompt = f"""
    Ти - професійний архітектор контенту. 
    Твоє завдання: написати сценарій на тему "{topic}" для "{audience}".
    
    ВАЖЛИВО: Перевір список існуючих категорій (slug): {existing_slugs}
    Якщо тема "{topic}" фактично відноситься до однієї з існуючих категорій (наприклад, "8 березня" і "Жіночий день" - це одне й те саме), обов'язково використай ІСНУЮЧИЙ category_slug.
    Якщо тема зовсім нова, придумай новий лаконічний category_slug англійською.

    Відповідь СУВОРО JSON:
    {{
      "category_name": "Назва свята (уніфікована)",
      "category_slug": "slug-з-бази-або-новий",
